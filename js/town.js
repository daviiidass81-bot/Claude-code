...
const Town = (() => {
  ...
  const Cat = (() => {
    ...
    function update(dt) {
      ...
      if (c.follow && !['petted', 'eat', 'chase'].includes(c.state)) {
    ...
    function pet() {
      ...
      set('petted', 2.2); Sfx.purr && Sfx.purr(); hearts(4);
      const s = S(); s.pets = (s.pets || 0) + 1;
      addAff(3); Store.emit({ type: 'catPet', pets: s.pets });
      Panel.say(U.pick([I18N.t('Mimi schnurrt zufrieden.'), I18N.t('Mimi schmiegt sich an deine Hand.'), I18N.t('Mimi blinzelt dich glücklich an.')]));
    }
    function feed(kind) {
      const inv = Inv.get();
